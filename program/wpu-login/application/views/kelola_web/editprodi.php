<div class="container-fluid">
    <div class="col-md-6">
        <div class="card shadow">
            ...
            <div class="card-body">
                <form action="<?= base_url('superadmin/proseseditprodi') ?>" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <input name="akreditas" type="text" class="form-control" id="NamaKelas"
                            value="<?= $prodi['akreditas']; ?>">
                    </div>
                    <div class="form-group">
                        <label for="Formenu">Fakultas</label>
                        <select class="form-control" id="Formenu" name="id_fakultas">
                            <?php foreach ($fakultas as $s) : ?>
                            <option value="<?= $s['id'] ?>" <?php if ($prodi['id_fakultas'] == $s['id']) : ?> selected
                                <?php endif; ?>><?= $s['nama'] ?> (<?= $s['alias'] ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="mt-3">
                        <button type="submit" class="btn btn-primary py-0">Simpan</button>
                        <a href="<?= base_url('superadmin/prodi') ?>" class="btn btn-secondary py-0">Kembali</a>
                    </div>
                </form>
            </div>
        </div>
